/*
   ...
   $Id: wlWaveOutputSettingsPage.cpp,v 1.12 2011/01/21 17:50:26 vividos Exp $

*/
/*! \file wlWaveOutputSettingsPage.cpp
...
// wlWaveOutputSettingsPage methods

LRESULT wlWaveOutputSettingsPage::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
void wlWaveOutputSettingsPage::OnEnterPage()
{
   ...
   SendDlgItemMessage(IDC_WAVE_CHECK_RAWAUDIO, BM_SETCHECK, 
      mgr.queryValueInt(wlWaveRawAudioFile)==1? BST_CHECKED : BST_UNCHECKED );

   // set writewavex check
   SendDlgItemMessage(IDC_WAVE_CHECK_WAVEX, BM_SETCHECK, 
      mgr.queryValueInt(wlWaveWriteWavEx)==1? BST_CHECKED : BST_UNCHECKED );

   // set output format
   SendDlgItemMessage(IDC_WAVE_COMBO_OUTFMT, CB_SETCURSEL, mgr.queryValueInt(wlWaveOutputFormat));
   SendDlgItemMessage(IDC_WAVE_COMBO_FILEFMT, CB_SETCURSEL, mgr.queryValueInt(wlWaveFileFormat));
}

bool wlWaveOutputSettingsPage::OnLeavePage()
{
   // get settings manager
   wlSettingsManager &mgr = pui->getUISettings().settings_manager;

   // get raw audio check
   mgr.setValue(wlWaveRawAudioFile,
      BST_CHECKED==SendDlgItemMessage(IDC_WAVE_CHECK_RAWAUDIO, BM_GETCHECK) ? 1 : 0 );

   // get writewavex check
   mgr.setValue(wlWaveWriteWavEx,
   ...
   mgr.setValue(wlWaveOutputFormat,value);
   value = SendDlgItemMessage(IDC_WAVE_COMBO_FILEFMT, CB_GETCURSEL);
   mgr.setValue(wlWaveFileFormat,value);
