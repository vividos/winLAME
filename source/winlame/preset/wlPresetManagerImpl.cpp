/*
   ...
   $Id: wlPresetManagerImpl.cpp,v 1.25 2005/09/27 18:30:09 vividos Exp $

*/
/*! \file wlPresetManagerImpl.cpp
...
// needed includes
#include "stdafx.h"
#include "wlPresetManagerImpl.h"
#include "wlPropertyDlg.h"
#include <fstream>
...
wlPresetManagerInterface *wlPresetManagerInterface::getPresetManager()
{
   return new wlPresetManagerImpl;
}

// wlPresetManagerInterface methods

wlPresetManagerImpl::wlPresetManagerImpl()
{
   xmlchanged=false;
}

bool wlPresetManagerImpl::loadPreset(LPCTSTR filename)
{
   // remember filename
...
bool wlPresetManagerImpl::mergePreset(LPCTSTR filename)
{
   // not implemented
...
void wlPresetManagerImpl::savePreset()
{
   if (xmlchanged)
   {
...
void wlPresetManagerImpl::setFacility(LPCTSTR facname)
{
   USES_CONVERSION;
   facility = facname;
...
int wlPresetManagerImpl::getPresetCount()
{
   // return length of presets list
   return presets_list->size();
}

std::tstring wlPresetManagerImpl::getPresetName(int index)
{
...
std::tstring wlPresetManagerImpl::getPresetDescription(int index)
{
...
void wlPresetManagerImpl::setSettings(int index, wlSettingsManager &settings_mgr)
{
   // search for preset node
   cppxml::xmlnode_ptr node(cppxml::get_nodelist_item(*presets_list,index));
...
void wlPresetManagerImpl::setDefaultSettings(wlSettingsManager &settings_mgr)
{
   for(int i=wlVarFirst; i<wlVarLast; i++)
   {
...
void wlPresetManagerImpl::editSettingsDialog(int index)
{
   // search for preset node
   cppxml::xmlnode_ptr node(cppxml::get_nodelist_item(*presets_list,index));
   if (node.get()==NULL) return;
...
int wlPresetManagerImpl::GetGroupCount()
{
   return 1;
}

std::tstring wlPresetManagerImpl::GetGroupName(int group)
{
...
int wlPresetManagerImpl::GetItemCount(int group)
{
   return editing_preset->select_nodes("value")->size();
}

std::tstring wlPresetManagerImpl::GetItemName(int group, int index)
{
...
std::tstring wlPresetManagerImpl::GetItemValue(int group, int index)
{
...
void wlPresetManagerImpl::SetItemValue(int group, int index, std::tstring val)
{
   // search node
   cppxml::xmlnode_ptr node(editLookupNode(group,index));
...
cppxml::xmlnode_ptr wlPresetManagerImpl::editLookupNode(int group, int index)
{
