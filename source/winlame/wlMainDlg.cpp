/*
   ...
   $Id: wlMainDlg.cpp,v 1.56 2011/01/21 17:50:26 vividos Exp $
 ...
 m_langResourceManager(_T("winlame.*.dll"), IDS_LANG_ENGLISH, IDS_LANG_NATIVE)
{
   ...
   settings.preset_manager = wlPresetManagerInterface::getPresetManager();
   ...
   settings.module_manager = wlModuleManager::getNewModuleManager();

   // insert some pages
   pages.push_back(new wlInputPage);
   pages.push_back(new wlOutputPage);
   pages.push_back(new wlEncodePage);
}
...
LRESULT wlMainDlg::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
   // center main window
   CenterWindow();
...
LRESULT wlMainDlg::OnSysCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
   if ((wParam & 0xFFF0)==IDM_ABOUTBOX)
   {
      // show about dialog
      wlAboutDlg dlg;
      dlg.SetModuleManager(settings.module_manager);
   ...
   {
      COptionsDlg dlg(settings, m_langResourceManager);
      dlg.DoModal(m_hWnd);
   }
   else
      bHandled = false;
...
void wlMainDlg::DrawCaptionBar(HDC &hDC, RECT &rc)
{
...
bool wlMainDlg::ActivatePage(int page)
{
   // activates a page
   ...
   if (currentpage != -1)
   {
      ...
      wlPageBase* oldpage = pages[oldpage_index];
   ...
   wlPageBase *newpage = pages[currentpage];
   newpage->pui = this;
...
CString wlMainDlg::GetWinlameDir()
{
   // get command line
...
void wlMainDlg::GetCommandLineFiles()
{
   // get command line
   LPTSTR lpszCommandLine = ::GetCommandLine();
   ...
   while(lpszCommandLine[0]!=0)
   {
      ...
      else
      {
         ...
         settings.encoderjoblist.push_back(wlEncoderJob(fname));
      }
...
LRESULT wlMainDlg::OnSize(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
   bHandled = false;
