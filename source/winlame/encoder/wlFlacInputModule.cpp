/*
   ...
   $Id: wlFlacInputModule.cpp,v 1.11 2010/06/28 18:51:27 vividos Exp $

*/
/*! \file wlFlacInputModule.cpp
...
//   context->streaminfo = metadata->data.stream_info;
   ...
   case FLAC__METADATA_TYPE_VORBIS_COMMENT:
      for (unsigned i = 0; i < metadata->data.vorbis_comment.num_comments; i++)
      {
         ...
         {
            context->trackInfo->TextInfo(wlTrackInfoTitle, value);
         }
         else if (name.CompareNoCase(_T("artist")) == 0)
         {
            context->trackInfo->TextInfo(wlTrackInfoArtist, value);
         }
         else if (name.CompareNoCase(_T("album")) == 0)
         {
            context->trackInfo->TextInfo(wlTrackInfoAlbum, value);
         }
         else if (name.CompareNoCase(_T("comment")) == 0)
         {
            context->trackInfo->TextInfo(wlTrackInfoComment, value);
         }
         else if (name.CompareNoCase(_T("genre")) == 0)
         {
            if (!value.IsEmpty())
               context->trackInfo->TextInfo(wlTrackInfoGenre, value);
         }
         else if (name.CompareNoCase(_T("tracknumber")) == 0)
         {
            context->trackInfo->NumberInfo(wlTrackInfoTrack, _ttoi(value));
         }
         else if (name.CompareNoCase(_T("year")) == 0)
         {
            context->trackInfo->NumberInfo(wlTrackInfoYear, _ttoi(value));
         }
...
// wlFlacInputModule methods

wlFlacInputModule::wlFlacInputModule()
{
   module_id = ID_IM_FLAC;
}

wlInputModule *wlFlacInputModule::cloneModule()
{
   return new wlFlacInputModule;
}

bool wlFlacInputModule::isAvailable()
{
...
void wlFlacInputModule::getDescription(CString& desc)
{
   // format string
...
void wlFlacInputModule::getVersionString(CString& version, int special)
{
   HMODULE dll = ::LoadLibrary(_T("libFLAC.dll"));
   if (dll != NULL)
   {
...
CString wlFlacInputModule::getFilterString()
{
   CString cszFilter;
...
int wlFlacInputModule::initInput(LPCTSTR infilename, wlSettingsManager &mgr,
   wlTrackInfo &trackinfo, wlSampleContainer &samplecont)
{
   ...
   // set up input traits
   samplecont.setInputModuleTraits(context->streaminfo.bits_per_sample,wlSamplesChannelArray,
      context->streaminfo.sample_rate,context->streaminfo.channels);

   return 0;
}

void wlFlacInputModule::getInfo(int &channels, int &bitrate, int &length, int &samplerate)
{
...
int wlFlacInputModule::decodeSamples(wlSampleContainer &samples)
{
