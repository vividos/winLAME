/*
   ...
   $Id: wlBassInputModule.cpp,v 1.12 2009/12/17 18:49:51 vividos Exp $

*/
/*! \file wlBassInputModule.cpp
...
// wlBassInputModule methods

wlBassInputModule::wlBassInputModule()
{
   module_id = ID_IM_BASS;
}

wlInputModule *wlBassInputModule::cloneModule()
{
   return new wlBassInputModule;
}

bool wlBassInputModule::isAvailable()
{
...
void wlBassInputModule::getDescription(CString& desc)
{
   // format string
   if (!is_str) // MOD file
   {
...
void wlBassInputModule::getVersionString(CString& version, int special)
{
   version.Empty();
...
CString wlBassInputModule::getFilterString()
{
...
int wlBassInputModule::initInput(LPCTSTR infilename, wlSettingsManager &mgr,
   wlTrackInfo &trackinfo, wlSampleContainer &samplecont)
{
   ...
   if (is_str && (info.ctype & BASS_CTYPE_STREAM_WMA))
   {
      ...
      for (size_t i = 0; i < vecCommentIndices.size(); i++)
      {
         ...
         if (cszName == WMA_TITLE_TAG)    trackinfo.TextInfo(wlTrackInfoTitle, cszValue);    else
         if (cszName == WMA_AUTHOR_TAG)   trackinfo.TextInfo(wlTrackInfoArtist, cszValue);   else
         if (cszName == WMA_ALBUM_TAG)    trackinfo.TextInfo(wlTrackInfoAlbum, cszValue);    else
         if (cszName == WMA_DESC_TAG)     trackinfo.TextInfo(wlTrackInfoComment, cszValue);  else
         if (cszName == WMA_GENRE_TAG)    trackinfo.TextInfo(wlTrackInfoGenre, cszValue);    else
         if (cszName == WMA_YEAR_TAG)
         {
            int iYear = _ttoi(cszValue);
            trackinfo.NumberInfo(wlTrackInfoYear, iYear);
         }
         else
         if (cszName == WMA_TRACKNO_TAG)
         {
            int iTrackNo = _ttoi(cszValue);
            trackinfo.NumberInfo(wlTrackInfoTrack, iTrackNo);
         }
         else
...
void wlBassInputModule::getInfo(int &channels, int &bitrate, int &length, int &samplerate)
{
   channels = info.chans;
...
int wlBassInputModule::decodeSamples(wlSampleContainer &samples)
{
   int ret;
...
float wlBassInputModule::percentDone()
{
   const int PATLEN = 64;
   float done;
...
void wlBassInputModule::doneInput()
{
   BASS_Free();
   delete[] buffer;
}
