/*
   ...
   $Id: wlEncoderImpl.cpp,v 1.47 2011/01/25 19:32:38 vividos Exp $

*/
/*! \file wlEncoderImpl.cpp
...
// static wlEncoderInterface methods

wlEncoderInterface* wlEncoderInterface::getNewEncoder()
{
   return new wlEncoderImpl;
}
...
// wlEncoderImpl methods

void wlEncoderImpl::encode()
{
   lockAccess();
   ...
   wlSampleContainer sample_container;

   // empty description string
   desc.Empty();

   // get input and output modules
   wlModuleManagerImpl* modimpl = reinterpret_cast<wlModuleManagerImpl*>(mod_manager);
   wlInputModule* inmod = modimpl->chooseInputModule(infilename);
   wlOutputModule* outmod = modimpl->getOutputModule(out_module_id);
   ...
   if (handler!=NULL && res<0)
   {
      ...
      {
      case wlEncoderErrorHandler::wlContinue:
         break;
      case wlEncoderErrorHandler::wlSkipFile:
         error=1;
         goto skipfile;
         break;
      case wlEncoderErrorHandler::wlStopEncode:
         error=-1;
         goto skipfile;
         break;
      }
...
//         }
   ...
   // check if we only need copying a cd ripped file to another filename
   {
      ...
      {
         extern const int wlSndFileOutputFormat[];

         // we have the right input and output modules; now check if output
         // parameters are 44100 Hz, 16 bit, stereo
         if (settings_mgr->queryValueInt(wlWaveRawAudioFile)==0 &&
             settings_mgr->queryValueInt(wlWaveWriteWavEx)==0 &&
             wlSndFileOutputFormat[settings_mgr->queryValueInt(wlWaveOutputFormat)] == SF_FORMAT_PCM_16 &&
             settings_mgr->queryValueInt(wlWaveFileFormat) == 0/*FILE_WAV*/)
         {
            ...
            if (fRet == FALSE)
            {
               ...
               wlEncoderErrorHandler::wlErrorAction action = handler->handleError(outfilename, cszCaption, dwError, cszErrorMessage, true);
               switch (action)
               {
               case wlEncoderErrorHandler::wlContinue:
                  break;
...
void wlEncoderImpl::GenerateTempOutFilename(const CString& cszOriginalFilename, CString& cszTempFilename)
{
