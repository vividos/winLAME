/*
   ...
   $Id: WinampPluginInputModule.cpp,v 1.25 2009/12/17 18:49:51 vividos Exp $
...
// WinampPluginInputModule methods

WinampPluginInputModule::WinampPluginInputModule()
{
...
InputModule *WinampPluginInputModule::cloneModule()
{
   WinampPluginInputModule *inmod2 = new WinampPluginInputModule;
   inmod2->setDLLModuleHandle(mod);
   inmod2->free_mod = false;
   return inmod2;
}

void WinampPluginInputModule::setDLLModuleHandle(HMODULE themod)
{
   mod = themod;
...
void WinampPluginInputModule::configureModule()
{
   inmod->Config(NULL/*::GetActiveWindow()*/);
}

bool WinampPluginInputModule::isAvailable()
{
...
int WinampPluginInputModule::initInput(LPCTSTR infilename,
...
void WinampPluginInputModule::getDescription(CString& desc)
{
   desc.Format(_T("%s, %u Hz, %u channels, %u bits per sample"),
      module_name, g_samplerate, g_numchannels, g_bitspersamp);
}

CString WinampPluginInputModule::getFilterString()
{
   return filterstring;
}

void WinampPluginInputModule::getInfo(int &ch, int &bitrate, int &length, int &srate)
{
   ch = g_numchannels;
   srate = g_samplerate;
...
int WinampPluginInputModule::decodeSamples(SampleContainer &samples)
{
   if (!started)
   {
      // lock mutex
...
void WinampPluginInputModule::doneInput()
{
