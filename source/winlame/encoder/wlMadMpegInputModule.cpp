/*
   ...
   $Id: wlMadMpegInputModule.cpp,v 1.39 2011/01/23 17:00:24 vividos Exp $

*/
/*! \file wlMadMpegInputModule.cpp
...
// wlMadMpegInputModule methods

wlMadMpegInputModule::wlMadMpegInputModule()
{
   module_id = ID_IM_MAD;
...
wlInputModule *wlMadMpegInputModule::cloneModule()
{
   return new wlMadMpegInputModule;
}

bool wlMadMpegInputModule::isAvailable()
{
...
void wlMadMpegInputModule::getDescription(CString& desc)
{
   // mpeg version
...
void wlMadMpegInputModule::getVersionString(CString& version, int special)
{
...
CString wlMadMpegInputModule::getFilterString()
{
   CString cszFilter;
...
int wlMadMpegInputModule::initInput(LPCTSTR infilename,
...
*/
   ...
   samplecont.setInputModuleTraits(32,
      wlSamplesChannelArray, init_header.samplerate,
...
bool wlMadMpegInputModule::checkVbrInfoTag(unsigned char *buffer)
{
   // mpeg version
   int id = (buffer[1] >> 3) & 1;
...
bool wlMadMpegInputModule::GetId3v2TagInfos(const CString& cszFilename, wlTrackInfo& trackinfo)
{
   ...
   if (tag.IsFrameAvail(ID3::FrameId::Genre))
   {
      ...
      if (!cszValue.IsEmpty())
         trackinfo.TextInfo(wlTrackInfoGenre, cszValue);
   }

   return true;
}

void wlMadMpegInputModule::getInfo(int &channels, int &bitrate, int &length, int &samplerate)
{
...
int wlMadMpegInputModule::decodeSamples(wlSampleContainer &samples)
{
   if (eos) return 0;
   ...
   for(int i=0; i<ret; i++)
   {
      ...
      // scale to 32 bits
      //synth.pcm.samples[ch][i] = sample >> (MAD_F_FRACBITS + 1 - wlMadOutputBits);
...
/*
   ...
   for(int i=0; i<ret; i++)
   {
      ...
      synth.pcm.samples[0][i] = audio_linear_dither(wlMadOutputBits,sample,&left_dither,&stats);
   }
   ...
   for(int i=0; i<ret; i++)
   {
      ...
      synth.pcm.samples[1][i] = audio_linear_dither(wlMadOutputBits,sample,&right_dither,&stats);
   }
*/
...
void wlMadMpegInputModule::doneInput()
{
   if (istr.is_open())
   {
      // close file
