/*
   ...
   $Id: wlBassWmaOutputModule.cpp,v 1.12 2010/01/08 19:58:43 vividos Exp $

*/
/*! \file wlBassWmaOutputModule.cpp
...
// already in wlBassInputModule.cpp
//#if _MSC_VER < 1400
...
// wlBassWmaOutputModule methods

wlBassWmaOutputModule::wlBassWmaOutputModule()
{
   module_id = ID_OM_BASSWMA;
}

bool wlBassWmaOutputModule::isAvailable()
{
...
void wlBassWmaOutputModule::getDescription(CString& desc)
{
   // format string
...
void wlBassWmaOutputModule::prepareOutput(wlSettingsManager &mgr)
{
   brmode = mgr.queryValueInt(wlWmaBitrateMode);
   switch (brmode)
   {
   case 1: // VBR
      bitrate = mgr.queryValueInt(wlWmaQuality);
      break;
   case 0: // CBR
   default:
      bitrate = mgr.queryValueInt(wlWmaBitrate) * 1000;
      break;
   }
}

int wlBassWmaOutputModule::initOutput(LPCTSTR outfilename,
   ...
   wlSampleContainer &samplecont)
{
   ...
   // add track properties
   {
      ...
      prop = trackinfo.TextInfo(wlTrackInfoComment, bAvail);
      if (bAvail && !prop.IsEmpty())
      {
         BASS_WMA_EncodeSetTag(handle, "Description", T2CA(prop), BASS_WMA_TAG_ANSI);
      }

      iProp = trackinfo.NumberInfo(wlTrackInfoTrack, bAvail);
      if (bAvail && iProp != -1)
      {
         prop.Format(_T("%u"), iProp);
      ...
      prop = trackinfo.TextInfo(wlTrackInfoGenre, bAvail);
      if (bAvail && !prop.IsEmpty())
      {
   ...
   samplecont.setOutputModuleTraits(16, wlSamplesInterleaved);

   return 0;
}

int wlBassWmaOutputModule::encodeSamples(wlSampleContainer &samples)
{
   // get samples
   int numsamples=0;
...
void wlBassWmaOutputModule::doneOutput()
{
   // closes the file
