/*
   ...
   $Id: wlOggVorbisOutputModule.cpp,v 1.40 2010/01/08 19:58:43 vividos Exp $

*/
/*! \file wlOggVorbisOutputModule.cpp
...
// wlOggVorbisOutputModule methods

wlOggVorbisOutputModule::wlOggVorbisOutputModule()
{
...
bool wlOggVorbisOutputModule::isAvailable()
{
   bool avail=false;
...
void wlOggVorbisOutputModule::getDescription(CString& desc)
{
   switch (brmode)
   {
   case 0: // quality mode
...
void wlOggVorbisOutputModule::getVersionString(CString& version, int special)
{
...
int wlOggVorbisOutputModule::initOutput(LPCTSTR outfilename,
   ...
   wlSampleContainer &samplecont)
{
   ...
   switch(brmode = mgr.queryValueInt(wlOggBitrateMode))
   {
   case 0: // base quality mode
      base_quality = mgr.queryValueInt(wlOggBaseQuality)/1000.f;
      ret = vorbis_encode_init_vbr_func(&vi,channels,samplerate,base_quality);
      break;

   case 1: // variable bitrate mode
      {
         int min_bitrate = mgr.queryValueInt(wlOggVarMinBitrate);
         int max_bitrate = mgr.queryValueInt(wlOggVarMaxBitrate);
   ...
   case 2: // average bitrate mode
      {
         int average_bitrate = mgr.queryValueInt(wlOggVarNominalBitrate);
         ret = vorbis_encode_init_func(&vi,channels,samplerate,
   ...
   case 3: // constant bitrate mode
      {
         int const_bitrate = mgr.queryValueInt(wlOggVarNominalBitrate);
         ret = vorbis_encode_init_func(&vi,channels,samplerate,
   ...
   // add track properties
   {
      ...
      bool avail = false;
      prop = trackinfo.TextInfo(wlTrackInfoTitle, avail);
      ...
      prop = trackinfo.TextInfo(wlTrackInfoArtist, avail);
      if (avail && !prop.IsEmpty())
      {
         UnicodeToUTF8(prop, buffer);
      ...
      prop = trackinfo.TextInfo(wlTrackInfoAlbum, avail);
      if (avail && !prop.IsEmpty())
      {
         UnicodeToUTF8(prop, buffer);
      ...
      int iYear = trackinfo.NumberInfo(wlTrackInfoYear, avail);
      if (avail && iYear > 0)
      {
      ...
      prop = trackinfo.TextInfo(wlTrackInfoComment, avail);
      if (avail && !prop.IsEmpty())
      {
         UnicodeToUTF8(prop, buffer);
      ...
      int iTrack = trackinfo.NumberInfo(wlTrackInfoTrack, avail);
      if (avail && iTrack >= 0)
      {
      ...
      CString cszGenre = trackinfo.TextInfo(wlTrackInfoGenre, avail);
      if (avail)
      {
         if (!cszGenre.IsEmpty())
         {
   ...
   samplecont.setOutputModuleTraits(16,wlSamplesChannelArray,samplerate,channels);

   return 0;
}

int wlOggVorbisOutputModule::encodeSamples(wlSampleContainer &samples)
{
   // end of stream?
   if (eos) return 0;
...
void wlOggVorbisOutputModule::doneOutput()
{
