/*
   ...
   $Id: wlAacOutputModule.cpp,v 1.27 2010/01/08 19:58:41 vividos Exp $

*/
/*! \file wlAacOutputModule.cpp
...
// wlAacOutputModule methods

wlAacOutputModule::wlAacOutputModule()
{
   module_id = ID_OM_AAC;
}

bool wlAacOutputModule::isAvailable()
{
...
void wlAacOutputModule::getDescription(CString& desc)
{
   // get config
...
int wlAacOutputModule::initOutput(LPCTSTR outfilename,
   ...
   wlSampleContainer &samplecont)
{
   ...
   // set settings
   config->mpegVersion = mgr.queryValueInt(wlAacMpegVersion)==4 ? MPEG4 : MPEG2;

   int value = mgr.queryValueInt(wlAacObjectType);
   config->aacObjectType = value==1 ? LOW : value==2 ? LTP : MAIN;

   config->allowMidside = mgr.queryValueInt(wlAacAllowMS);
   config->useLfe = mgr.queryValueInt(wlAacUseLFEChan);
   config->useTns = mgr.queryValueInt(wlAacUseTNS);
   config->shortctl = SHORTCTL_NORMAL;
   config->inputFormat = FAAC_INPUT_16BIT;

   // set bandwidth
   if (mgr.queryValueInt(wlAacAutoBandwidth))
   {
      config->bandWidth = 0;
   }
   else
   {
      config->bandWidth = mgr.queryValueInt(wlAacBandwidth);
   }

   // set bitrate/quality
   brcmethod = mgr.queryValueInt(wlAacBRCMethod);
   if (brcmethod == 0) // Quality
   {
      config->quantqual = mgr.queryValueInt(wlAacQuality);
      config->bitRate = 0;
   }
   else // Bitrate
   {
      config->quantqual = 0;
      config->bitRate = mgr.queryValueInt(wlAacBitrate) * 1000 / channels;
   }
...
int wlAacOutputModule::encodeSamples(wlSampleContainer &samples)
{
   // get samples
   int numsamples=0;
...
void wlAacOutputModule::doneOutput()
{
   int ret=0;
