/*
   ...
   $Id: wlLameOutputModule.cpp,v 1.59 2010/01/08 19:58:43 vividos Exp $

*/
/*! \file wlLameOutputModule.cpp
...
nlame_instance_t *wlLameOutputModule::nogap_inst = NULL;


// wlLameOutputModule methods

wlLameOutputModule::wlLameOutputModule()
{
   inst = NULL;
   mp3buf = NULL;
...
bool wlLameOutputModule::isAvailable()
{
...
void wlLameOutputModule::getVersionString(CString& version, int special)
{
   USES_CONVERSION;
...
void wlLameOutputModule::prepareOutput(wlSettingsManager &mgr)
{
   waveheader = mgr.queryValueInt(wlLameWriteWaveHeader)!=0;
}

//! error callback
void wlLameErrorCallback(const char* format, va_list ap)
{
   USES_CONVERSION;
...
int wlLameOutputModule::initOutput(LPCTSTR outfilename,
   ...
   wlSampleContainer &samplecont)
{
   ...
   nogap = mgr.queryValueInt(wlLameOptNoGap)==1;
   ...
   // set callbacks
   nlame_callback_set(inst, nle_callback_error,wlLameErrorCallback);
   ...
   // set all nlame variables
   {
      ...
      // mono encoding?
      bool bMono = mgr.queryValueInt(wlLameSimpleMono) == 1;
      ...
      if (mgr.queryValueInt(wlLameSimpleQualityOrBitrate) == 0)
      {
         // bitrate mode
         int nBitrate = mgr.queryValueInt(wlLameSimpleBitrate);

         if (mgr.queryValueInt(wlLameSimpleCBR)==1)
         {
            // CBR
      ...
      else
      {
         ...
         int nQuality = mgr.queryValueInt(wlLameSimpleQuality);

         nlame_var_set_int(inst, nle_var_vbr_quality, nQuality);

         // VBR mode; wlLameSimpleVBRMode, 0: standard, 1: fast
         int nVbrMode = mgr.queryValueInt(wlLameSimpleVBRMode);
      ...
      // encode quality; wlLameSimpleEncodeQuality, 0: fast, 1: standard, 2: high
      // fast maps to quality factor 7, high is quality factor 2
      // note: this currently is hard coded; maybe move this into nlame?
      int nEncQuality = mgr.queryValueInt(wlLameSimpleEncodeQuality);
...
skip_nogap:
   ...
   // do description string
   {
      ...
      cszText.Format(IDS_FORMAT_INFO_LAME,
         ...
         mgr.queryValueInt(wlLameSimpleMono) == 0 ? _T("") : _T(" (mono)"));

      CString cszFormat;

      // quality or bitrate mode?
      int nQualityOrBitrate = mgr.queryValueInt(wlLameSimpleQualityOrBitrate);
      if (nQualityOrBitrate == 0) // bitrate
      {
         int nBitrate = mgr.queryValueInt(wlLameSimpleBitrate);
         cszFormat.Format(
            mgr.queryValueInt(wlLameSimpleCBR) == 0 ? IDS_FORMAT_INFO_LAME_ABR : IDS_FORMAT_INFO_LAME_CBR,
            nBitrate);
      ...
      else // quality
      {
         ...
         cszFormat.Format(IDS_FORMAT_INFO_LAME_VBR,
            nQuality,
            mgr.queryValueInt(wlLameSimpleVBRMode) == 0 ? _T("Standard") : _T("Fast"));
         cszText += cszFormat;
      }
...
//   infotag = mgr.queryValueInt(wlLameVBRWriteTag)==1;
   ...
   if (!trackinfo.IsEmpty())
      id3tag = new wlId3v1Tag(trackinfo);
   ...
   samplecont.setOutputModuleTraits(bps,wlSamplesInterleaved);
...
*/
int wlLameOutputModule::encodeFrame()
{
...
int wlLameOutputModule::encodeSamples(wlSampleContainer& samples)
{
   // get samples
   int numsamples=0;
...
void wlLameOutputModule::doneOutput()
{
...
void wlLameOutputModule::AddLameID3v2Tag(const wlTrackInfo& trackinfo)
{
   ...
   USES_CONVERSION;
   bool bAvail = false;
   CString cszValue = trackinfo.TextInfo(wlTrackInfoTitle, bAvail);
   if (bAvail)
      nlame_id3tag_setfield_latin1(inst, nif_title, T2CA(cszValue));

   cszValue = trackinfo.TextInfo(wlTrackInfoArtist, bAvail);
   if (bAvail)
      nlame_id3tag_setfield_latin1(inst, nif_artist, T2CA(cszValue));

   cszValue = trackinfo.TextInfo(wlTrackInfoComment, bAvail);
   if (bAvail)
      nlame_id3tag_setfield_latin1(inst, nif_comment, T2CA(cszValue));

   cszValue = trackinfo.TextInfo(wlTrackInfoAlbum, bAvail);
   if (bAvail)
      nlame_id3tag_setfield_latin1(inst, nif_album, T2CA(cszValue));

   // numeric
   int iValue = trackinfo.NumberInfo(wlTrackInfoYear, bAvail);
   if (bAvail)
   {
   ...
   iValue = trackinfo.NumberInfo(wlTrackInfoTrack, bAvail);
   if (bAvail)
   {
      cszValue.Format(_T("%i"), iValue);
   ...
   cszValue = trackinfo.TextInfo(wlTrackInfoGenre, bAvail);
   if (bAvail)
   {
...
unsigned int wlLameOutputModule::GetID3v2PaddingLength()
{
   unsigned int uiLength = 0;

   CString cszValue;
   bool bAvail;
   const wlTrackInfo& trackinfo = m_trackInfoID3v2;

   // add all frames
   cszValue = trackinfo.TextInfo(wlTrackInfoTitle, bAvail);
   if (bAvail)
...
void wlLameOutputModule::WriteID3v2Tag()
{
