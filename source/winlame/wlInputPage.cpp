/*
   ...
   $Id: wlInputPage.cpp,v 1.55 2011/01/21 17:50:26 vividos Exp $

*/
/*! \file wlInputPage.cpp
...
// needed includes
#include "stdafx.h"
#include "wlInputPage.h"
#include "wlEncoderInterface.h"
#include "encoder/wlModuleInterface.h"
...
// wlInputConfigDlg methods

void wlInputConfigDlg::Init(wlModuleManager* mymgr, std::vector<int> myindices)
{
   mgr = mymgr;
   indices = myindices;
}

LRESULT wlInputConfigDlg::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
   int max = indices.size();
   for(int i=0; i<max; i++)
   {
      wlInputModule* mod = mgr->getInputModuleInstance(indices[i]);
...
LRESULT wlInputConfigDlg::OnListBoxDoubleClick(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
...
LRESULT wlInputPage::OnInitDialog(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
   ...
   wlModuleManager* module_manager = pui->getUISettings().module_manager;
   int max = module_manager->getInputModuleCount();
   for(int j=0; j<max; j++)
   {
      wlInputModule* module = module_manager->getInputModuleInstance(j);
   ...
   if (!wlCDRipDlg::IsCDExtractionAvail())
...
LRESULT wlInputPage::OnButtonDeleteAll(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
...
void wlInputPage::DeleteSelectedFromCDRipTrackManager()
{
...
void wlInputPage::UpdateTimeCount()
{
...
LRESULT wlInputPage::OnButtonConfig(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
   if (config_modules_id.empty())
      return 0;

   // start config selection dialog
   wlInputConfigDlg dlg;
...
LRESULT wlInputPage::OnButtonCDRip(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
   // first check if cd ripping is available
   if (!wlCDRipDlg::IsCDExtractionAvail())
      return 0;

   wlCDRipDlg dlg(pui->getUISettings(), *pui);
...
LRESULT wlInputPage::OnDropFiles(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT wlInputPage::OnKeyDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT wlInputPage::OnListItemChanged(int idCtrl, LPNMHDR pnmh, BOOL& bHandled)
{
...
LRESULT wlInputPage::OnDoubleClickedList(int idCtrl, LPNMHDR pnmh, BOOL& bHandled)
{
...
LRESULT wlInputPage::OnButtonPlay(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
...
void wlInputPage::PlayFile(LPCTSTR filename)
{
   ::ShellExecute( NULL, _T("play"), filename, NULL, NULL, SW_SHOW);
}

void wlInputPage::OpenFileDialog()
{
   // get filter string
   CString filter;
...
void wlInputPage::InsertFilename(LPCTSTR filename)
{
...
void wlInputPage::OnEnterPage()
{
   ...
   wlEncoderJobList& fnlist = pui->getUISettings().encoderjoblist;
   int max = fnlist.size();
   ...
   for(int i=0;i<max;i++)
   {
      ...
      if (0 == filename.find(g_pszCDRipPrefix))
      {
         ...
         wlCDRipTrackInfo& trackinfo = pManager->GetTrackInfo(nIndex);
...
bool wlInputPage::OnLeavePage()
{
   ...
   for(int i=0;i<max;i++)
   {
      ...
      fnlist.push_back(wlEncoderJob(fname));
   }
...
void wlInputPage::ImportM3uPlaylist(LPCTSTR filename)
{
   // open playlist
   USES_CONVERSION;
...
void wlInputPage::ImportPlsPlaylist(LPCTSTR filename)
{
   // open playlist
   USES_CONVERSION;
   std::ifstream plist(T2CA(filename),std::ios::in);
...
void wlInputPage::ImportCueSheet(LPCTSTR filename)
{
   // open cue sheet
   USES_CONVERSION;
...
void wlInputPage::InsertFileWithIcon(LPCTSTR pszRealFilename, LPCTSTR pszFilenameForIcon,
