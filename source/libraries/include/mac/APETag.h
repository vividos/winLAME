...
public:
	...
	CAPETagField(const wchar_t * pFieldName, const void * pFieldValue, int nFieldBytes = -1, int nFlags = 0);
...
private:
	...
	CSmartPtr<wchar_t> m_spFieldNameUTF16;
...
public:
	...
	CAPETag(const wchar_t * pFilename, BOOL bAnalyze = TRUE);
	...
	int SetFieldString(const wchar_t * pFieldName, const wchar_t * pFieldValue);
	int SetFieldString(const wchar_t * pFieldName, const char * pFieldValue, BOOL bAlreadyUTF8Encoded);
	int SetFieldBinary(const wchar_t * pFieldName, const void * pFieldValue, int nFieldBytes, int nFieldFlags);

	// gets the value of a field (returns -1 and an empty buffer if the field doesn't exist)
	int GetFieldBinary(const wchar_t * pFieldName, void * pBuffer, int * pBufferBytes);
	int GetFieldString(const wchar_t * pFieldName, wchar_t * pBuffer, int * pBufferCharacters);
	int GetFieldString(const wchar_t * pFieldName, char * pBuffer, int * pBufferCharacters, BOOL bUTF8Encode = FALSE);

	// remove a specific field
	int RemoveField(const wchar_t * pFieldName);
	...
	CAPETagField * GetTagField(const wchar_t * pFieldName);
...
private:
	...
	// private functions
	int	Analyze();
	int GetTagFieldIndex(const wchar_t * pFieldName);
	...
	int SetFieldID3String(const wchar_t * pFieldName, const char * pFieldValue, int nBytes);
	int GetFieldID3String(const wchar_t * pFieldName, char * pBuffer, int nBytes);
